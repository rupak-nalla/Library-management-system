...
async function fetchRequests(){
     ...
     const res= await fetch("http://localhost:8080/api/user/requests",{
          method:"GET",
          headers: {
...
<template>
     <div id="main" class="row" style="color: white;">
          ...
          <div class="col" style="padding: 20px;">
               <h3>My Requests</h3>
               <div>
                    <div class="card" v-for="i in reqs.length" style="margin: 5px;">
                         <div class="card-body" style="display: inline;">
                              <div style="float: left;">
                                   <h5>{{reqs[i-1].BName}}</h5>
                              </div>
                              <div style="float: right;">
                                   <h5> Status: {{ reqs[i-1].Status }}</h5>
                              </div>
                         </div>
                    </div>
               </div>

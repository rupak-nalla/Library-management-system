...
//     secBooks.value = secData.value['books'];
//     console.log(secBooks.value);
...
async function fetchSectionData() {
     ...
     if (res.status == 200) {
          ...
          if (result['books'].length > 0) {
               for (let i = 0; i < result['books'].length; i += 2) {
                    temp.push(result['books'][i]);
                    temp.push(result['books'][i + 1]);
                    temp2.push(temp);
                    temp = [];
               }
               ...
               secData.value = result;
               secBooks.value=temp2
               console.log(secBooks.value);
          } else {
...
async function fetchBooks() {
     const token = localStorage.getItem('token');
     const res= await fetch("http://localhost:8080/api/sectionCreate",{
          method:"GET",
          headers: {
               'Authorization': `Bearer ${token}`,
               'Content-Type':'application/json'
...
async function handleSubmit() {
     ...
     var checkboxesToDelete = document.querySelectorAll('input[name="optionTodelete"]:not(:checked)');
     
     const secName = document.getElementById('SectionName').value;
     var checkedValues = [];
     ...
     if (checkedValues.length == 0 && unCheckedValues.length==0) {
     alert('Select/unselect a book');
     } else {
          const body = {
               'sid':sid,
               'sname': secName,
               'selectedBids': checkedValues,
               'unselectedBids':unCheckedValues
          };
          const token = localStorage.getItem('token');
...
function check(book){
     for(let i=0;i<secBooks.value.length;i++){
          if(book.Bid==secBooks.value[i].Bid){
               return true
          }
     }
     return false
...
<template>
     <div id="main" class="row">
          ...
          <div class="col" style="margin: 10px;">
               <h3 style="color: white;">Edit Section</h3>
               <div class="mb-3" style="display: flex;">
                    <!-- <label for="" class="form-label" style="color: white;">Section Name</label> -->
                    <input
               ...
               <h3 style="color: white;">Books in Section</h3>
               <div class="row" v-for="i in  secBooks.length">
                         
                         <div class="col">
                              <div class="card mb-3" style="max-width: 540px;">
                                   <div class="row g-0">
                                        <div class="col-md-4">
                                             <img :src="secBooks[i-1][0].CoverPath" class="img-fluid rounded-start" alt="...">
                                        </div>
                                        <div class="col-md-8">
                                             <div class="card-body">
                                                  <h5 class="card-title">{{ secBooks[i-1][0].Name }}</h5>
                                                  <p class="card-text">
                                                       <strong>Author</strong>: {{ secBooks[i-1][0].AuthorName }}<br>
                                                  </p>
                                                  <div>
                                                       <div class="form-check">
                         
                                                            <input class="form-check-input" type="checkbox" :value="secBooks[i-1][0].Bid" id="bookSelected" name="optionTodelete" checked>
                         ...
                         <div class="col">
                              <div v-if="secBooks[i-1][1]" class="card mb-3" style="max-width: 540px;">
                                   <div class="row g-0">
                                        <div class="col-md-4">
                                             <img :src="secBooks[i-1][1].CoverPath" class="img-fluid rounded-start" alt="...">
                                        </div>
                                        <div class="col-md-8">
                                             <div class="card-body">
                                                  <h5 class="card-title">{{ secBooks[i-1][1].Name }}</h5>
                                                  <p class="card-text">
                                                       <strong>Author</strong>: {{ secBooks[i-1][1].AuthorName }}<br>
                                                  </p>
                                                  <div>
                                                       <div class="form-check">
                                                            <input class="form-check-input" type="checkbox" :value="secBooks[i-1][1].Bid" id="bookSelected" name="optionTodelete" checked>
                                                       </div>
                                                  </div>
               ...
               <!-- define bookData and fetch the books remember once a book is alloted it can't be used again -->
               <h3 style="color: white;">Books to add</h3>
               <div class="row" v-if="booksData.length>0" v-for="i in  booksData.length">
                                             ...
                                             <div class="card-body">
                                                  ...
                                                  <div>
                                                       <div class="form-check">
                                                            ...
                                                            <input class="form-check-input" type="checkbox" :value="booksData[i-1][0].Bid" id="bookSelected" name="option">
                                   ...
                                   <div class="row g-0">
                                        ...
                                        <div class="col-md-8">
                                             <div class="card-body">
                                                  ...
                                                  <div>
                                                       <div class="form-check">
                                                            <input class="form-check-input" type="checkbox" :value="booksData[i-1][1].Bid" id="bookSelected" name="option" >
                                                       </div>
                                                  </div>
               ...
               <div class="row" v-else>
                    <h3 class="color:white;">No books to add</h3>
               </div>
          </div>
     </div>
</template>
